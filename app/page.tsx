...
const decisionRates: HistoryRow[] = modelRows.map((row) => ({ d: row.d, r: 1 / row.rubPerKzt }));
...
function RateTooltip({ active, payload }: { active?: boolean; payload?: Array<{ payload: HistoryRow }> }) {
  ...
  return <div className="chart-tooltip"><span>{compactDate(row.d)}</span><strong>{row.r.toFixed(2)} ₸ за 1 ₽</strong></div>;
}
...
export default function Home() {
  ...
  const factRows = rateHistory.slice(Math.max(0, historyIndex - 59), historyIndex + 1);
  ...
  const userPercentile = Math.round((factRows.filter((row) => row.r <= selectedRate.r).length / factRows.length) * 100);
  const fiveSessionsAgo = rateHistory[Math.max(0, historyIndex - 5)];
  const fiveSessionChange = (selectedRate.r / fiveSessionsAgo.r) - 1;

  const clientTitle = clientState === 'no-session'
    ? 'На выбранную дату нового решения нет'
    : clientState === 'none'
      ? 'Уведомление о курсе не отправляется'
      : fiveSessionChange < 0
        ? `За 5 сессий курс снизился на ${Math.abs(fiveSessionChange * 100).toFixed(1).replace('.', ',')}%`
        : `За 5 сессий курс вырос на ${(fiveSessionChange * 100).toFixed(1).replace('.', ',')}%`;
  const clientDescription = clientState === 'no-session'
    ...
    : clientState === 'none'
      ? `Текущий курс находится выше, чем в ${userPercentile}% последних наблюдений. На экране остаются только факты о динамике.`
      : `Текущий курс находится выше, чем в ${userPercentile}% последних наблюдений. Это проверяемый исторический факт, а не прогноз.`;
  ...
  const modelRules = [
    { name: 'Порог NOW', active: nowScorePassed, detail: `${score(selectedSession.probability)} ${nowScorePassed ? '≥' : '<'} ${score(modelProfile.nowThreshold)}` },
    { name: 'Cooldown', active: cooldownPassed, detail: previousContact ? `${sessionGap} сесс. после контакта · нужно > ${modelProfile.cooldownSessions}` : 'предыдущих контактов нет' },
    { name: 'Лимит недели', active: weeklyLimitPassed, detail: `${contactsThisWeek} из 2 контактов до этой сессии` },
    { name: 'Порог CLOSING', active: closingScorePassed, detail: `${score(selectedSession.closingProbability)} ${closingScorePassed ? '≥' : '<'} ${score(modelProfile.closingThreshold)}` },
    { name: 'Направление ret1', active: positiveRet1, detail: `${percent(selectedSession.ret1, 2)} · RUB за KZT` },
  ];
  const verdict = selectedSession.candidate ? (selectedSession.closing ? 'NOW + CLOSING' : 'NOW') : 'NONE';
  const featureFacts = [
    { icon: selectedSession.ret5 >= 0 ? TrendingUp : TrendingDown, title: `Изменение за 5 сессий · ${percent(selectedSession.ret5, 1)}`, text: 'Признак ret5 в направлении RUB за KZT, использованный моделью.' },
    { icon: Sparkles, title: `Положение за 60 сессий · ${Math.round(selectedSession.pr60 * 100)}%`, text: 'Значение признака pr60 в модельном срезе — без отдельного ручного порога.' },
    { icon: TrendingUp, title: `Волатильность vol20 · ${score(selectedSession.vol20)}`, text: 'Историческая изменчивость за 20 сессий — один из входов TabM.' },
  ];
  ...
  return (
    <main className="app-shell">
      <header className="topbar">
        ...
        <div className="header-meta"><span className="live-dot" aria-hidden="true" />RUB → KZT · История курса</div>
      </header>
      <div id="top" className="content">
        <section className="intro" aria-labelledby="page-title"><div><div className="overline">Переводы · RUB → KZT</div><h1 id="page-title">Что происходило с курсом?</h1><p>Выберите дату — покажем клиентский экран и точное решение финальной модели.</p></div></section>
        ...
        <section className="control-grid" aria-label="Выбор даты и результат">
          <div className="card date-card">
            ...
            <div className="preset-row" aria-label="Быстрые сценарии"><span>Примеры:</span><button type="button" onClick={() => setSelectedDate('2026-05-06')}>NOW</button><button type="button" onClick={() => setSelectedDate('2026-06-02')}>NOW + CLOSING</button><button type="button" onClick={() => setSelectedDate('2026-06-04')}>порог без контакта</button></div>
          </div>
        ...
        <section className="card chart-card" aria-labelledby="chart-title">
          <div className="chart-head"><div><div className="card-kicker">Динамика вокруг даты</div><h2 id="chart-title">Сколько тенге за 1 рубль</h2></div><div className="chart-summary"><div className="rate-now"><strong>{selectedRate.r.toFixed(2).replace('.', ',')} ₸</strong><span>на модельную сессию</span></div><span className={`period-change ${chartChange >= 0 ? 'positive' : 'negative'}`}>{percent(chartChange, 2)}</span></div></div>
          ...
          <div className="range-panel" aria-label="Диапазон курса за 3 месяца">
            <div className="range-head"><div><div className="card-kicker">3 месяца до выбранной даты</div><strong>Позиция курса в диапазоне</strong></div><div className={`range-change ${threeMonthChange >= 0 ? 'positive' : 'negative'}`}>{threeMonthChange >= 0 ? <TrendingUp size={17} /> : <TrendingDown size={17} />}{percent(threeMonthChange)}</div></div>
            <div className="range-metrics"><div><span>Минимум</span><strong>{threeMonthMin.toFixed(2).replace('.', ',')} ₸</strong></div><div className="range-current"><span>Выбранный курс</span><strong>{selectedRate.r.toFixed(2).replace('.', ',')} ₸</strong></div><div><span>Максимум</span><strong>{threeMonthMax.toFixed(2).replace('.', ',')} ₸</strong></div></div>
            ...
            <div className="range-caption"><span>{compactDate(threeMonthRows[0].d)}</span><span>выше минимума на {((selectedRate.r / threeMonthMin - 1) * 100).toFixed(1).replace('.', ',')}%</span><span>{compactDate(selectedSession.d)}</span></div>
          </div>
        </section>
        ...
        <section className="facts-section" aria-labelledby="facts-title">
          ...
          <article className="card model-overview">
            ...
            <div className="trigger-block"><div className="card-kicker">Фактические условия решения</div><div className="trigger-grid">{modelRules.map((rule) => <div className={`trigger-item ${rule.active ? 'active' : ''}`} key={rule.name}><span>{rule.active ? <Check size={14} aria-hidden="true" /> : <CircleMinus size={14} aria-hidden="true" />}{rule.name}</span><small>{rule.active ? 'Выполнено' : 'Не выполнено'} · {rule.detail}</small></div>)}</div></div>
          </article>
          ...
          <div className="facts-grid">{featureFacts.map((fact) => { const FactIcon = fact.icon; return <article className="card fact-card" key={fact.title}><div className="fact-icon"><FactIcon size={21} /></div><h3>{fact.title}</h3><p>{fact.text}</p></article>; })}</div>
        </section>
